# RIZKY PRATAMA

*Fresh graduate Teknik Informatika — tertarik pada data, QA, dan produk digital*

rizky.pratama@contoh.example  |  0813-0000-1234  |  Jakarta, Indonesia  |  linkedin.com/in/rizkypratama-contoh

## Ringkasan

Lulusan baru S1 Teknik Informatika dengan pengalaman magang QA di startup aplikasi Android dan dua proyek kampus berbasis data. Terbiasa bekerja dengan spreadsheet dan SQL untuk kebutuhan laporan, teliti, dan cepat belajar tool baru.

## Pengalaman

**Magang QA Tester** — PT Aplikasi Kita, Jakarta · Jun–Agu 2025 (3 bulan)
- Menjalankan test case manual untuk aplikasi Android (fitur pendaftaran, pembayaran, notifikasi) — 120+ test case per rilis
- Menyusun laporan bug di spreadsheet dan Jira: langkah reproduksi, tangkapan layar, tingkat keparahan
- Mendampingi rilis mingguan bersama 2 developer; ikut regression test sebelum publish ke Play Store

**Data Analyst (freelance, remote) untuk 3 klien UMKM** — 3 klien UMKM · Jan 2025–Jun 2026 (1,5 tahun)
- Membangun laporan penjualan bulanan dengan SQL (PostgreSQL) dan Python (pandas), dashboard sederhana di Google Sheets.

## Proyek

**Tugas akhir — sistem rekomendasi menu kantin kampus** — proyek kampus · 2026
- Membangun basis data MySQL dari 3 bulan data transaksi kantin (8.000 baris) dan query laporan penjualan harian
- Model rekomendasi sederhana berbasis item yang sering dibeli bersama (Python, pandas); dokumentasi dan pengujian

**Dashboard kehadiran himpunan** — proyek kampus · 2024
- Google Sheets + Apps Script untuk rekap kehadiran 140 anggota; grafik otomatis per divisi

## Pendidikan

**S1 Teknik Informatika** — Universitas Nusantara Jaya, Jakarta · 2022–2026 · IPK 3,10 / 4,00
Mata kuliah relevan: Basis Data, Pemrograman Web, Statistika, Rekayasa Perangkat Lunak

## Keahlian

- **Bahasa pemrograman:** Python, SQL, JavaScript (dasar)
- **Tools:** Microsoft Excel, Google Sheets, Figma, Git, Jira
- **Bahasa:** Indonesia (native), Inggris (menengah, TOEFL ITP 520)

## Organisasi

**Ketua Divisi Acara** — Himpunan Mahasiswa Informatika · 2024
Mengelola 4 acara kampus (80–300 peserta), tim 12 orang, sponsor 3 perusahaan
